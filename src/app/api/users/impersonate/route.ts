...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const [userToImpersonate] = await db.select().from(users).where(eq(users.id, userId))
        ...
        const response = NextResponse.json({ message: 'Impersonation successful' }, { status: 200 })
        response.cookies.set('next-auth.session-token', JSON.stringify(impersonatedToken), {
            httpOnly: true,
